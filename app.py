...
def score_stock(df, sym, mode, vix_val, htf_up, rs_rank, phase_history_sym=None):
    cfg     = MODE_CFG[mode]
    cl      = df["Close"]
    hi      = df["High"]
    lo      = df["Low"]
    ...
    bull += phase_transition_conf_bonus(sym)
    ...
    _, _, _, _ = record_phase_transition(sym, phase)
...
def run_scan(symbols: list, mode: str, vix_val, htf_cache: dict,
    ...
    def _process_one(sym):
        ...
        with lock:
            sym_returns[sym] = r52
        ph = st.session_state.get("phase_history", {}).get(sym, [])
        rs_rank = 50
...
def run_exit_scan(positions: list, mode: str, vix_val, htf_cache: dict) -> list:
    results = []
    lock    = threading.Lock()
    phase_history_all = st.session_state.get("phase_history", {})
