...
with tab_scanner:
    ...
    if st.session_state.results:
        ...
        def make_card(i, r, border_color, show_entry=True):
            """
            Full-width stacked card matching the reference design:
            ┌─────────────────────────────────────────────────────┐
            │  [01] SYMBOL    [BUY]  74.2                    🔖   │  ← header
            ├──────────────────────┬──────────────────────────────┤
            │  ₹339.05            │  [BREAKOUT]  [HIGH 84%] [RS97]│  ← body
            │  +6.09% ▲           │  ⚠ EMA overext               │
            │  ⚡ ₹331.71         │                              │
            ├──────────────────────┴──────────────────────────────┤
            │  RSI 48.9  |  ↑ Bullish  |  Vol High  |  Sector    │  ← footer
            └─────────────────────────────────────────────────────┘
            """
            chg = r["%Change"]
            ...
            # ── Number badge bg (matches action) ──
            ...
            # ── Phase icon ──
            phase_icon = {"BREAKOUT":"📈","CONT":"🔄","ENTRY":"⚡","SETUP":"🔍","IDLE":"💤","EXIT":"🚪"}.get(ph, "")
