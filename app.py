"""NSE Master Scanner Pro — Streamlit EditionMode-aware (Intraday / Swing / Positional) with Phase State Machine

Fixes applied:FIX-1: Positional qualified gate — demoted from hard block to score penaltyFIX-2: TTL comment was stale (already 300s, caption corrected)FIX-3: Intraday momentum now uses a separate daily fetch; 5m bars excludedFIX-4: EMA/RSI/ATR computed once in score_stock, passed into detect_phaseFIX-5: Fib SL uses max() not min() — was giving absurdly wide stopsFIX-6: detect_phase receives pre-computed norm_bull so phase & action use identical inputs"""

import warningsimport loggingfrom datetime import datetime

import numpy as npimport pandas as pdimport yfinance as yfimport streamlit as st

from nse500 import nse500_symbolsfrom sectors import SECTORS

logging.getLogger("yfinance").setLevel(logging.CRITICAL)warnings.filterwarnings("ignore")

NIFTY500 = list(dict.fromkeys([s.strip().upper().replace(".NS", "") for s in nse500_symbols]))for k in SECTORS:if SECTORS[k] is None:SECTORS[k] = NIFTY500

── Mode config ──────────────────────────────────────────────────

MODE_CFG = {"Intraday":   dict(period="5d",  interval="5m",  ema_fast=20, ema_slow=50,atr_mult=1.5, atr_wide=3.0, atr_max=1.0,mom1_th=2,  mom3_th=5,  mom6_th=8,  score_th=65, rsi_len=14),"Swing":      dict(period="1y",  interval="1d",  ema_fast=50, ema_slow=200,atr_mult=2.5, atr_wide=4.0, atr_max=1.5,mom1_th=3,  mom3_th=7,  mom6_th=10, score_th=70, rsi_len=21),"Positional": dict(period="2y",  interval="1d",  ema_fast=50, ema_slow=200,atr_mult=3.5, atr_wide=5.0, atr_max=1.5,mom1_th=5,  mom3_th=10, mom6_th=15, score_th=70, rsi_len=21),}

Phase constants

PHASE_IDLE  = "IDLE"PHASE_SETUP = "SETUP"PHASE_ENTRY = "ENTRY"PHASE_CONT  = "CONT"PHASE_BRK   = "BREAKOUT"PHASE_EXIT  = "EXIT"

PHASE_COLORS = {PHASE_IDLE:  "#555577",PHASE_SETUP: "#b87333",PHASE_ENTRY: "#2255cc",PHASE_CONT:  "#22aa55",PHASE_BRK:   "#00dd88",PHASE_EXIT:  "#cc4444",}

── Core math ────────────────────────────────────────────────────

def to_nse(sym):sym = sym.strip().upper()return sym if sym.endswith(".NS") else sym + ".NS"

def ema(s, p):return s.ewm(span=p, adjust=False).mean()

def rsi(s, p=14):d = s.diff()g = d.clip(lower=0).ewm(alpha=1/p, adjust=False).mean()l = (-d.clip(upper=0)).ewm(alpha=1/p, adjust=False).mean()return 100 - (100 / (1 + g / l.replace(0, np.nan)))

def atr_series(df, p=14):hi, lo, cl = df["High"], df["Low"], df["Close"]tr = pd.concat([(hi-lo), (hi-cl.shift()).abs(), (lo-cl.shift()).abs()], axis=1).max(axis=1)return tr.ewm(alpha=1/p, adjust=False).mean()

def fib_levels(df, lookback=30):sw_hi = float(df["High"].iloc[-lookback:].max())sw_lo = float(df["Low"].iloc[-lookback:].min())rng   = sw_hi - sw_loif rng == 0:return sw_hi, sw_lo, {}, rngreturn sw_hi, sw_lo, {"236":    sw_hi - rng * 0.236,"382":    sw_hi - rng * 0.382,"500":    sw_hi - rng * 0.500,"618":    sw_hi - rng * 0.618,"786":    sw_hi - rng * 0.786,"ext127": sw_hi + rng * 0.272,"ext161": sw_hi + rng * 0.618,"ext261": sw_hi + rng * 1.618,}, rng

def _compute_targets(entry, sl, atr_val, fib, setup_type, sw_hi, sw_lo):rk = max(entry - sl, atr_val * 0.5)
...
@st.cache_data(ttl=900)def _fetch_daily_close(ticker):"""Fetch 6 months of daily closes for HTF momentum — used by Intraday mode only."""try:df = yf.download(ticker, period="6mo", interval="1d",auto_adjust=True, progress=False, threads=False)if isinstance(df.columns, pd.MultiIndex):df.columns = df.columns.get_level_values(0)return df["Close"].dropna()except Exception:return pd.Series(dtype=float)
...
def detect_phase_and_entry(df, mode, *,c, e_fast_s, e_slow_s, atr_s, atr_val, atr_mean,v, vol_avg, fib, sw_hi, sw_lo, in_golden, near_e127, near_e161,norm_bull, trend_up, trend_down, trend_strong, score_th,vdu_setup=False,   # Volume Dry-Up coil — enables early SETUP promotion):cfg  = MODE_CFG[mode]close = df["Close"]high  = df["High"]n     = len(close)if n < 60:return PHASE_IDLE, None, "norm"
...
def score_stock(df, nifty_close, mode="Swing", daily_close=None):"""daily_close: pre-fetched daily Close series (only used when mode==Intradayto compute HTF momentum on proper daily bars)."""try:cfg    = MODE_CFG[mode]close  = df["Close"]volume = df["Volume"]n      = len(close)if n < 50:return None
...
def fetch_nifty(mode="Swing"):cfg = MODE_CFG[mode]df  = yf.download("^NSEI", period=cfg["period"], interval=cfg["interval"], progress=False)if isinstance(df.columns, pd.MultiIndex):df.columns = df.columns.get_level_values(0)return df["Close"].dropna()

def _market_regime(nifty_close):"""Returns (market_bullish, regime_label).market_bullish=True  → full scoring.market_bullish=False → bearish; apply score haircut + surface a warning.Gate: Nifty close > EMA50  AND  EMA20 > EMA50."""if len(nifty_close) < 50:return True, "UNKNOWN"ema20_val = float(ema(nifty_close, 20).iloc[-1])ema50_val = float(ema(nifty_close, 50).iloc[-1])bullish   = (float(nifty_close.iloc[-1]) > ema50_val) and (ema20_val > ema50_val)return bullish, "BULLISH" if bullish else "BEARISH"

def _fetch_one(args):"""Worker for ThreadPoolExecutor — download OHLCV for a single ticker."""sym, mode, min_bars = argscfg    = MODE_CFG[mode]ticker = to_nse(sym)try:df = yf.download(ticker, period=cfg["period"], interval=cfg["interval"],auto_adjust=True, progress=False, threads=False)if df.empty:return sym, Noneif isinstance(df.columns, pd.MultiIndex):df.columns = df.columns.get_level_values(0)df = df.dropna(how="all")if pd.isna(df["Close"].iloc[-1]):df = df.iloc[:-1]df["Close"]  = df["Close"].ffill()df["Volume"] = df["Volume"].fillna(0)df = df.dropna(subset=["Close"])return sym, (df if len(df) >= min_bars else None)except Exception:return sym, None

def run_scan(symbols, mode, progress_bar, status_text):"""Improvements applied here:• Market regime filter  — bearish Nifty haircuts all scores by 15 %• Async/parallel fetch  — ThreadPoolExecutor replaces sequential loop• FIX-3 (HTF momentum) — daily_closes pre-fetched in parallel too"""import concurrent.futures
...
def action_label_fn(s):"""Standalone action label used after regime haircut."""if s >= 100: return "STRONG BUY"if s >= 80:  return "BUY"if s >= 60:  return "WATCH"return "SKIP"

def fmt(val):if val is None or (isinstance(val, float) and np.isnan(val)):return "—"return f"₹{val:,.2f}"

def action_icon(a):return {"STRONG BUY":"🟢","BUY":"🔵","WATCH":"🟡","SKIP":"🔴"}.get(a,"")

── Streamlit UI ─────────────────────────────────────────────────

st.set_page_config(page_title="NSE Master Scanner Pro", page_icon="📈",layout="wide", initial_sidebar_state="collapsed")st.markdown("""

st.title("📈 NSE Master Scanner Pro  [Phase Engine v5]")

for key, default in [("results",[]),("scan_time",None),("rejected",0),("scan_mode","Swing"),("zd_enctoken",""),("zd_ltp_cache",{})]:if key not in st.session_state:st.session_state[key] = default
...
c1, c2, c3, c4, c5, c6 = st.columns([2,1,1,2,2,2])with c1:index_opt = st.selectbox("Index", list(SECTORS.keys()), label_visibility="collapsed")with c2:mode_opt = st.selectbox("Mode", ["Swing","Intraday","Positional"], label_visibility="collapsed")
...
ZERODHA_QUOTE_URL = "https://api.kite.trade/quote"ZERODHA_LTP_URL   = "https://api.kite.trade/quote/ltp"

NSE instrument tokens for indices (fixed, don't change)

ZERODHA_INDEX_TOKENS = {"Nifty 50":  "NSE 50","Sensex":    "BSE","BankNifty": "NSE BANK",}

def _zerodha_headers(enctoken):return {"Authorization": f"enctoken {enctoken.strip()}","Content-Type":  "application/json","X-Kite-Version": "3",}

@st.cache_data(ttl=15)   # 15-sec cache — live quotedef zd_fetch_index_quote(enctoken, instruments):"""Fetch live quotes for a list of instrument strings like ['NSE 50']."""import requeststry:params = [("i", inst) for inst in instruments]r = requests.get(ZERODHA_QUOTE_URL,headers=_zerodha_headers(enctoken),params=params, timeout=5)if r.status_code == 200:return r.json().get("data", {})return {}except Exception:return {}

@st.cache_data(ttl=15)def zd_fetch_ltp_bulk(enctoken, instrument_list):"""Fetch LTP for up to 500 instruments in one call.instrument_list: ['NSE', 'NSE', ...]Returns dict {'NSE': 2950.5, ...}"""import requestsout = {}CHUNK = 500for i in range(0, len(instrument_list), CHUNK):chunk = instrument_list[i+CHUNK]try:params = [("i", inst) for inst in chunk]r = requests.get(ZERODHA_LTP_URL,headers=_zerodha_headers(enctoken),params=params, timeout=8)if r.status_code == 200:data = r.json().get("data", {})for k, v in data.items():out[k] = v.get("last_price", None)except Exception:passreturn out

def zd_index_display(quote_data, instrument_key, name):"""Extract display fields from Zerodha quote response for one index.Returns dict compatible with fetch_indices() output format,augmented with ohlc, week52_high/low, volume."""d = quote_data.get(instrument_key)if not d:return Noneltp    = d.get("last_price", 0)ohlc   = d.get("ohlc", {})prev   = ohlc.get("close", ltp)chg    = round(ltp - prev, 2)pct    = round((chg / prev) * 100, 2) if prev else 0return {"value":    ltp,"chg":      chg,"pct":      pct,"open":     ohlc.get("open"),"high":     ohlc.get("high"),"low":      ohlc.get("low"),"prev":     prev,"source":   "Zerodha",}

def _enctoken_widget():"""Renders the one-time enctoken input box.Shows step-by-step instructions on how to get the token.Returns the token string or ''."""if "zd_enctoken" not in st.session_state:st.session_state["zd_enctoken"] = ""
...
Open kite.zerodha.com and log in

Press F12 → Application tab → Cookies → kite.zerodha.com

Find the cookie named enctoken — copy its value

Paste below ↓

Token resets daily around 6 AM IST. Re-paste if quotes stop updating.Your token is stored only in this browser session — never sent anywhere except Zerodha's own servers.""")col_inp, col_btn = st.columns([5, 1])with col_inp:token_input = st.text_input("enctoken", type="password",value=st.session_state["zd_enctoken"],placeholder="Paste your Zerodha enctoken here…",label_visibility="collapsed",)with col_btn:save_btn = st.button("✅ Save", use_container_width=True)
...
@st.cache_data(ttl=180)   # refresh every 3 min — OI data is near-real-timedef fetch_oi_data(symbol="NIFTY"):"""Fetch weekly expiry OI data from NSE option chain API.Returns dict with: expiry, spot, pcr, max_pain, call_wall, put_wall,top_ce_strikes, top_pe_strikes, oi_table (DataFrame).Falls back to None on any error (NSE blocks bots outside market hours).
...
def _oi_sentiment(pcr):if pcr >= 1.3:  return "Bullish 🟢", "#2ecc71"if pcr >= 0.9:  return "Neutral 🟡", "#f39c12"return "Bearish 🔴", "#e74c3c"

def _render_oi_card(oi, index_name):"""Render OI + Max Pain section inside an expander for one index."""if oi is None:st.caption(f"⚠️ OI data unavailable for {index_name} — NSE API may be closed or rate-limited.")return
...
@st.cache_data(ttl=300)def fetch_indices(mode="Swing"):cfg      = MODE_CFG[mode]ema_f    = cfg["ema_fast"]ema_s    = cfg["ema_slow"]rsi_l    = cfg["rsi_len"]min_bars = 30 if mode == "Intraday" else 50out = {}for name, ticker in [("Nifty 50","^NSEI"),("Sensex","^BSESN")]:try:df = yf.download(ticker, period=cfg["period"], interval=cfg["interval"], progress=False)if isinstance(df.columns, pd.MultiIndex):df.columns = df.columns.get_level_values(0)df = df.dropna()if len(df) < min_bars: out[name]=None; continueclose = df["Close"]c, prev = float(close.iloc[-1]), float(close.iloc[-2])chg, pct = c-prev, (c-prev)/prev100ef   = float(ema(close, ema_f).iloc[-1])es   = float(ema(close, ema_s).iloc[-1])e200 = float(ema(close, 200).iloc[-1]) if len(close)>=200 else esr    = float(rsi(close, rsi_l).iloc[-1])hh   = float(close.iloc[-11:-1].max())trend_up = c > e200 and c > ef and ef > esbull  = 0bull += 25 if trend_up else 0bull += 30 if ef > es else (20 if ef > es0.995 else 0)bull += (25 if r>=65 else 20) if r>=60 else (10 if r>50 else 0)bull += 25 if c>hh else (15 if c>hh*0.98 else 0)if len(close)>=3 and c>float(close.iloc[-3]): bull+=10score  = bullaction = "STRONG BUY" if score>=100 else ("BUY" if score>=80 else ("WATCH" if score>=60 else "SKIP"))interval_label = {"5m":"5min","1d":"Daily","1wk":"Weekly"}.get(cfg["interval"], cfg["interval"])out[name] = {"value","chg","pct","score","action","rsi"(r,1),"trend":"↑ Above EMAs" if trend_up else "↓ Below EMAs","interval": interval_label,"ema_fast": ema_f, "ema_slow": ema_s}except Exception:out[name] = Nonereturn out

indices = fetch_indices(mode_opt)

── OI data ───────────────────────────────────────────────────────

oi_nifty  = fetch_oi_data("NIFTY")oi_sensex = fetch_oi_data("BANKNIFTY")

── Live index quotes (Zerodha if token present, else yfinance) ───

zd_index_data = {}if enctoken:_zd_raw = zd_fetch_index_quote(enctoken,[ZERODHA_INDEX_TOKENS["Nifty 50"], ZERODHA_INDEX_TOKENS["Sensex"]])for idx_name, inst_key in [("Nifty 50", ZERODHA_INDEX_TOKENS["Nifty 50"]),("Sensex",   ZERODHA_INDEX_TOKENS["Sensex"])]:zd_index_data[idx_name] = zd_index_display(_zd_raw, inst_key, idx_name)

ic1, ic2, ic3 = st.columns([2,2,6])for col, name, oi_sym in zip([ic1, ic2],["Nifty 50", "Sensex"],[oi_nifty, oi_sensex]):# Prefer Zerodha live quote; fall back to yfinancezd = zd_index_data.get(name)d  = indices.get(name)with col:# Resolve display valuesif zd:ltp_val = zd["value"]chg_val = zd["chg"]pct_val = zd["pct"]ohlc_str = (f'O:{zd["open"]:,.0f}  H:{zd["high"]:,.0f}  'f'L:{zd["low"]:,.0f}  P:{zd["prev"]:,.0f}'if zd.get("open") else "")src_badge = ('🟢 LIVE')elif d:ltp_val  = d["value"]chg_val  = d["chg"]pct_val  = d["pct"]ohlc_str = ""src_badge = ('yfinance')else:st.markdown(f"{name}: unavailable"); continue
...
with ic3:live_note = "🟢 Live via Zerodha (15-sec refresh)" if enctoken else "📡 yfinance (5-min cache)"st.caption(f"{live_note} · OI refreshes every 3 min · Technical scores via yfinance")

── OI Detail expanders ───────────────────────────────────────────

oi_x1, oi_x2 = st.columns(2)with oi_x1:with st.expander("📊 Nifty 50 — Weekly OI & Max Pain", expanded=False):_render_oi_card(oi_nifty, "NIFTY")with oi_x2:with st.expander("📊 Bank Nifty — Weekly OI & Max Pain (NSE proxy for Sensex)", expanded=False):_render_oi_card(oi_sensex, "BANKNIFTY")

with c3:scan_btn = st.button("🔍 SCAN", type="primary", use_container_width=True)with c4:filter_opt = st.selectbox("Show",["BUY + STRONG BUY","STRONG BUY only","WATCH + BUY","All Results"],label_visibility="collapsed")with c5:phase_filter = st.selectbox("Phase Filter",["All Phases","ENTRY","SETUP","CONT","BREAKOUT","IDLE","EXIT"],label_visibility="collapsed")with c6:search_q = st.text_input("Search", placeholder="e.g. RELIANCE", label_visibility="collapsed")

Mode badge

mc = {"Intraday":"#e67e22","Swing":"#27ae60","Positional":"#2980b9"}mi = {"Intraday":"⚡","Swing":"📈","Positional":"🧘"}cfg_cur = MODE_CFG[mode_opt]interval_label = {"5m":"5min candles","1d":"Daily candles","1wk":"Weekly candles"}.get(cfg_cur["interval"], cfg_cur["interval"])last_info = (f"  "f"{st.session_state.scan_time} · Rejected: {st.session_state.rejected}"if st.session_state.scan_time else "")intraday_note = ("  HTF momentum via daily data"if mode_opt == "Intraday" else "")st.markdown(f''f'<span style="background:{mc.get(mode_opt,"#555")};color:#fff;'f'padding:3px 12px;border-radius:12px;font-size:12px;font-weight;">'f'{mi.get(mode_opt,"")} {mode_opt} · {interval_label} · 'f'EMA{cfg_cur["ema_fast"]}/{cfg_cur["ema_slow"]}'f'{intraday_note}{last_info}',unsafe_allow_html=True)

── Scan ─────────────────────────────────────────────────────────

if scan_btn:symbols = SECTORS[index_opt]n   = len(symbols)est = "~1 min" if n<=50 else ("~2 mins" if n<=150 else "3–5 mins")prog = st.progress(0)stat = st.empty()with st.spinner(f"Scanning {index_opt} ({n} stocks) · {mode_opt} mode · {est}"):results, rejected = run_scan(symbols, mode_opt, prog, stat)st.session_state.results   = resultsst.session_state.rejected  = rejectedst.session_state.scan_mode = mode_optst.session_state.scan_time = datetime.now().strftime("%H:%M:%S") + f" ({index_opt} · {mode_opt})"
...
if filter_opt == "BUY + STRONG BUY":results = [r for r in results if r["Action"] in ("BUY","STRONG BUY")]elif filter_opt == "STRONG BUY only":results = [r for r in results if r["Action"] == "STRONG BUY"]elif filter_opt == "WATCH + BUY":results = [r for r in results if r["Action"] in ("WATCH","BUY","STRONG BUY")]

if phase_filter != "All Phases":results = [r for r in results if r.get("Phase") == phase_filter]

if search_q:results = [r for r in results if search_q.upper() in r["Symbol"]]

── Top cards ────────────────────────────────────────────────────

if st.session_state.results:all_results = st.session_state.resultsACTIONABLE_PHASES = {PHASE_ENTRY, PHASE_CONT, PHASE_BRK}actionable = [r for r in all_resultsif r.get("Phase") in ACTIONABLE_PHASES and r["Action"] in ("BUY","STRONG BUY")]phase_rank = {PHASE_BRK: 0, PHASE_CONT: 1, PHASE_ENTRY: 2}actionable.sort(key=lambda x: (phase_rank.get(x.get("Phase"), 9), -x["Score"]))top_act = actionable[:15]
...
if results:sb = sum(1 for r in results if r["Action"]=="STRONG BUY")b  = sum(1 for r in results if r["Action"]=="BUY")w  = sum(1 for r in results if r["Action"]=="WATCH")sk = sum(1 for r in results if r["Action"]=="SKIP")pe = sum(1 for r in results if r.get("Phase")==PHASE_ENTRY)pb = sum(1 for r in results if r.get("Phase")==PHASE_BRK)m1,m2,m3,m4,m5,m6,m7 = st.columns(7)m1.metric("Showing",    len(results))m2.metric("🟢 Str Buy", sb)m3.metric("🔵 Buy",     b)m4.metric("🟡 Watch",   w)m5.metric("🔴 Skip",    sk)m6.metric("📍 ENTRY",   pe)m7.metric("🚀 BRK",     pb)

── Main table ────────────────────────────────────────────────────

if results:rows = []for i, r in enumerate(results):chg   = r["%Change"]phase = r.get("Phase", PHASE_IDLE)entry_flag = " ⚡" if r["Entry"] != r["LTP"] else ""setup_icon = {"fib":"🌀","breakout":"🚀","norm":"📊","vdu":"🔕"}.get(r.get("Setup","norm"),"📊")rows.append({"#":       i+1,"Symbol":  r["Symbol"],"Score":   r["Score"],"Phase":   phase,"Setup":   f'{setup_icon} {r.get("Setup","norm")}',"Action":  f"{action_icon(r['Action'])} {r['Action']}","%Chg":    f"+{chg}%" if chg>=0 else f"{chg}%","LTP":     fmt(r["LTP"]),"Entry":   fmt(r["Entry"]) + entry_flag,"SL":      fmt(r["SL"]),"T1":      fmt(r["T1"]),"T2":      fmt(r["T2"]),"T3":      fmt(r["T3"]),"Golden":  "🌟" if r.get("InGolden") else "","VDU":     "🔕" if r.get("VDU") else "","Regime":  r.get("Regime", "—"),})st.dataframe(pd.DataFrame(rows), use_container_width=True, hide_index=True, height=480)st.caption("⚡ Entry = signal trigger price (fib zone / breakout / EMA cross). LTP = current price.")
...
else:if not st.session_state.results:st.info("👆 Select Index + Mode, then press SCAN to begin.")else:st.warning("No stocks match the current filters.")
